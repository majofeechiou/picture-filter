...
(function body (window) {
	...
	class ImageProcess{
		...
		getData( src, painter_style ){
			...
			if( _me.promise!==undefined ){
				delete _me.promise;
			}

			_me.promise = new Promise( function (resolve, reject) {

				if( _me.obj_image===undefined ){

					_me.obj_image = new Image();

			        _me.obj_image.onload = function () {
			        ...
			        _me.obj_image.onerror = function () {
						reject({
							success: true,
			        ...
			        _me.obj_image.crossOrigin = null;
					
			    }

		        _me.obj_image.src = src ;
